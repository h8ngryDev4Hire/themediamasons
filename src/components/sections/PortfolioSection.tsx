      ...
      >
        ...
        <div className="w-full h-full">
          ...
          >
            ...
            <div className={`absolute top-0 left-0 right-0 z-30 p-6 flex items-center justify-between backdrop-blur-md bg-gradient-to-b from-black/70 to-transparent transition-opacity duration-700 ${
              showOverlay ? 'opacity-0' : 'opacity-100'
            }`}>
              <div className="flex items-baseline gap-4">
                <h3 className={`${raleway.className} text-2xl font-bold`}>{currentProject.title}</h3>
                <p className="text-gray-300">Client: {currentProject.client}</p>
              </div>
            ...
            <div 
              className={`absolute bottom-0 left-0 right-0 z-30 transition-all duration-500 ${
                showTestimonial ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-full pointer-events-none'
              }`}
              onMouseEnter={() => handleTestimonialVisibility(true)}
              onMouseLeave={() => handleTestimonialVisibility(false)}
            >
              <div className="backdrop-blur-xl p-6 bg-gradient-to-t from-media-mason-purple/70 to-black/60 border-t border-white/20 shadow-lg">
                <div className="flex items-start">
                  ...
                  <div>
                    <p className="text-md italic mb-2">{currentProject.testimonial.quote}</p>
